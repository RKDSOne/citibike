...
MANHATTAN <- MANHATTAN + 
  geom_point(aes(x=lon, y=lat), data=trips.sum, col="green", alpha=.5, size=trips.sum$size) +
  ggtitle("Gender 0") + 
  theme(plot.title=element_text(family="Times", face="bold", size=40))
print(MANHATTAN)


png(filename="output/gender_0.png",width=1000,height=1000)
plot(MANHATTAN)
dev.off()
